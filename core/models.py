...
from utils import django_extras
...
class TaskDependency(models.Model):
    ...
    parent_task = generic.GenericForeignKey('_parent_task_content_type', '_parent_task_object_id')
    ...
    child_task = generic.GenericForeignKey('_child_task_content_type', '_child_task_object_id')
    ...
    def is_satisfied(self, iteration):
        ...
        if self.get_parent().is_deleted():
            ...
            raise TMSInvalidStateException("Parent %s of %s is deleted. Tree for Job '%s' is broken!" \
        ...
        #    raise TMSInvalidStateException("Parent '%s' of '%s' is expired w/ no status. Tree for Job '%s' broken!" \
...
class Task(models.Model):
    ...
    ALL_TASK_TYPES = (TASK_TYPE_PERSISTENT, TASK_TYPE_EPHEMERAL, TASK_TYPE_SCHEDULED)
    
    class Meta:
        abstract = True
    
    #
    # values for these are to be given upon instantiation by the implementing classes:
    #
    
    task_type = models.CharField(choices=(zip(ALL_TASK_TYPES, ALL_TASK_TYPES)), max_length=16, default=TASK_TYPE_PERSISTENT)
    
    #
    # these are managed by the Task superclass (this class)
    #
    current_run_status = None       # to efficiently change status w/o having to read from db
    
    job = models.ForeignKey('Job')
    # To find this Task's parents, look where this Task is the child in the dependency table
    parent_dependencies = generic.GenericRelation('TaskDependency'
    ...
    #child_dependencies = generic.GenericRelation('TaskDependency'
    #                                        , content_type_field='_parent_task_content_type'
    #                                        , object_id_field='_parent_task_object_id')
    run_statuses = generic.GenericRelation('TaskRunStatus'
                                            , content_type_field='_task_content_type'
                                            , object_id_field='_task_object_id')
    resource_relationships = generic.GenericRelation('TaskResourceRelationship'
                                                    , content_type_field='_task_content_type'
                                                    , object_id_field='_task_object_id')
    ...
    def __try_reserve_resources__(self, region):
        """
        ...
        if not reserved_all:
            ...
            for rr in reserved:
                rr.release(region)
        return reserved_all
    def __release_resources__(self, region):
        for rr in self.resource_relationships.all():
            rr.release(region)
    ...
    def __set_run_status__(self, iteration, status, tmsd_status=None):
    ...
    def set_ended_on_error(self, iteration, region):
        self.__set_run_status__(iteration, TaskRunStatus.STATUS_ERROR)
        self.__release_resources__(region)
    def set_ended_on_timeout(self, iteration, region):
        self.__set_run_status__(iteration, TaskRunStatus.STATUS_TIMEDOUT)
        self.__release_resources__(region)
    ...
    def do_run(self, iteration, tmsd_status):
        ...
        try:
            try:
                if not self.__try_reserve_resources__(tmsd_status.get_region()):
                    raise InsufficientResourcesException()
                self.__set_run_status__(iteration, TaskRunStatus.STATUS_RUNNING, tmsd_status=tmsd_status)
                log.info("Running Task '%s'" % (self))
                success = self.run()
                if success:
                    self.__set_run_status__(iteration, TaskRunStatus.STATUS_SUCCESS)
            ...
            try:
                self.__release_resources__(tmsd_status.get_region())
            except:
                pass
...
class Iteration(models.Model):
    ...
    STATUS_DONE = 'DONE'             # Iteartion is done; no more Tasks will ever be run
...
class SchedulableTask(Task):
    ...
    def is_allowed_to_run(self, iteration, asof=None):
        ...
        if self.get_task_type() == SchedulableTask.TASK_TYPE_SCHEDULED:
            ...
            raise TMSInvalidStateException("Task type '%s' for '%s' is unsupported for SchedulableTasks." \
...
class ResourceReservation(models.Model):
    """
    A reservation of a resource in a given region in tms
    """
...
class TMSInvalidStateException(Exception):
    pass
...
# TODO change this to not have 'daemon' in the name; they can be run not as a daemon.
#      TaskControllerStatus or somesuch
class NorcDaemonStatus(models.Model):
    """Norc Daemons fire off tasks as they become due to run. Keep track of them here."""
    
    DAEMON_TYPE_TMS = 'TMS'
    DAEMON_TYPE_SQS = 'SQS'
    
    STATUS_STARTING = 'STARTING'                    # Daemon is starting.
    STATUS_ERROR = TaskRunStatus.STATUS_ERROR       # Daemon itself exited with bad error
    STATUS_RUNNING = TaskRunStatus.STATUS_RUNNING   # Daemon is currently running
    STATUS_PAUSEREQUESTED = 'PAUSEREQUESTED'        # Pause has been requested
    STATUS_PAUSED = 'PAUSED'                        # Don't launch any more tasks, just wait.
    STATUS_STOPREQUESTED = 'STOPREQUESTED'          # Don't launch any more tasks, and exit gracefully when running tasks have completed.
    STATUS_STOPINPROGRESS = 'BEING_STOPPED'         # The daemon has received a stop request and will exit gracefully.
    STATUS_KILLREQUESTED = 'KILLREQUESTED'          # Don't launch any more tasks, and kill already running tasks with error status.
    STATUS_KILLINPROGRESS = 'BEING_KILLED'          # The daemon has received a kill request and is shutting down.
    STATUS_ENDEDGRACEFULLY = 'ENDED'                # Daemon was exited gracefully; no tasks were interrupted upon shutting down.
    STATUS_KILLED = 'KILLED'                        # Daemon was killed; any running tasks were interrupted.
    STATUS_DELETED = Task.STATUS_DELETED            # Daemon status has been hidden for convenience.
